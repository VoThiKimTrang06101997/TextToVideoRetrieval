...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, data in tqdm (enumerate(self.train_data_loader), total=total_batches, desc="Processing batches"):  
...
def load_checkpoint(model, optimizer, checkpoint_dir):
    ...
    # Sort checkpoints by epoch number and load the latest one
...
if __name__ == "__main__":
    ...
    # Check if there is a checkpoint and load the latest one
    start_epoch = load_checkpoint(model, optimizer, config.model_path)

    # Training loop (resume from start_epoch if needed)
    ...
    # Save final checkpoint after the last epoch
